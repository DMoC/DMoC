...
namespace caba {
	...
	class VciMccCache : public soclib::caba::BaseModule
	{
		...
		enum req_fsm_state_e{
			...
			REQ_TLB_INVAL,				// On TLB invalidation, build an Acknownledge paquet
			REQ_TLB_INVAL_SEND_ACK, // Send the Acknownledge paquet for the TLB invalidation
			REQ_TLB_IMISS,		// Instruction TLB miss request
			REQ_TLB_DMISS,		// Data TLB miss request
		...
		enum rsp_fsm_state_e{
			...
			RSP_TLB_IMISS,		// Process TLB instruction miss
			RSP_TLB_DMISS,		// Process TLB data miss
			RSP_INVAL_SEND_ACK, // Process response (ack) for the sended TLB inval. Acknownledge
		};
		...
		enum inv_fsm_state_e{
			...
			INV_CMD_TLB, 		// Process TLB invalidation 
		...
		const unsigned int m_PAGE_SHIFT;
		...
		TLB_32< (unsigned int)vci_param::N >				m_TLB;
		...
		sc_signal<typename vci_param::addr_t>	r_REQ_DCACHE_ADDR_VIRT;	// address for VCI request 
		...
		// Regarding TLB invalidation
		sc_signal<typename vci_param::addr_t>	r_TLB_INV_TAG_0;		// tag entry 0 of the TLB that must be invalidated
		sc_signal<typename vci_param::addr_t>	r_TLB_INV_TAG_1;		// tag entry 1 of the TLB that must be invalidated, two tags entry to avoid deadlocks
		sc_signal<bool>		r_IS_TLB_INV_0;		// flag to select between invalidate TLB and invalidate a cache line
		sc_signal<bool>		r_IS_TLB_INV_1;		// flag to select between invalidate TLB and invalidate a cache line, (for the two buffered requests 0 and 1)
		sc_signal<typename vci_param::addr_t>	r_TLB_INV_ACK_ADDR;	// physical address where to send the tag invalidation ack
