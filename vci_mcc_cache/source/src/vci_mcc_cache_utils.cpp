...
tmpl(void)::gen_output_stats(void)
{
	...
	file_stats << std::dec << m_id  << " data tlb hit " << (stats.dcache_tlb_hit - stats.dcache_tlb_miss) << std::endl;
	file_stats << std::dec << m_id  << " data tlb miss " << (stats.dcache_tlb_miss) << std::endl;
	file_stats << std::dec << m_id  << " data tlb miss ratio " << (float)(stats.dcache_tlb_miss * 100.0)/(float)(stats.dcache_tlb_hit) << std::endl;
	file_stats << std::dec << m_id  << " data tlb nack " << (stats.dcache_tlb_nack) << std::endl;
	file_stats << std::dec << m_id  << " dtlb mean latency " << ((float)(stats.dcache_tlb_delay)/(float)(stats.dcache_tlb_miss)) << std::endl;

	file_stats << std::dec << m_id  << " inst tlb hit " << (stats.icache_tlb_hit - stats.icache_tlb_miss) << std::endl;
	file_stats << std::dec << m_id  << " inst tlb miss " << (stats.icache_tlb_miss) << std::endl;
	file_stats << std::dec << m_id  << " inst tlb nack " << (stats.icache_tlb_nack) << std::endl;
	file_stats << std::dec << m_id  << " inst tlb miss ratio " << (float)(stats.icache_tlb_miss * 100.0)/(float)(stats.icache_tlb_hit) << std::endl;
	file_stats << std::dec << m_id  << " itlb mean latency " << ((float)(stats.icache_tlb_delay)/(float)(stats.icache_tlb_miss)) << std::endl;
	...
	file_stats << std::dec << m_id  << " total_dist_virt_reqs : " << std::dec << stats. total_dist_virt_reqs << std::endl;
	file_stats << std::dec << m_id  << " moyenne distance virt en hops : "
		<< std::dec << (float)((float) stats. total_dist_virt_reqs / (float)stats.total_reqs) << std::endl;
	...
	file_stats << std::dec << m_id << " warning: latency is latency of misses, this not includes hits " << std::endl;
	file_stats << std::dec << m_id  << " warning: nacks does not include ll/sc/unc/tlb access" << std::endl;
	file_stats.close();
}
#endif
}}
