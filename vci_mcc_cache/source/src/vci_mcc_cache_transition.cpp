...
tmpl(void)::transition()
{
	...
	switch ((req_fsm_state_e)r_VCI_REQ_FSM.read())
	{
		case REQ_IDLE :
			{
				...
				if (r_DCACHE_REQ) // A Data cache request 
				{
					...
					{ // accessing a memory node (tty, fdacces etc returns -1)
						stats . total_dist_reqs += m_table_cost[address_to_id(m_TLB.translate(r_DCACHE_ADDR_SAVE.read()))];
						stats . total_dist_virt_reqs += m_table_cost[address_to_id(r_DCACHE_ADDR_SAVE.read())];
						stats . total_reqs += 1;
					}
#endif
				...
				else if (r_DCACHE_FLUSH_WB_REQ) // A Write-buffer flush request
				{
					...
					{
						stats . total_dist_reqs += m_table_cost[address_to_id(m_TLB.translate(m_WRITE_BUFFER_ADDR.read()))];
						stats . total_dist_virt_reqs += m_table_cost[address_to_id(m_WRITE_BUFFER_ADDR.read())];
						stats . total_reqs += 1;
					}
#endif
				...
				else if (r_ICACHE_MISS_REQ) // Else, process instruction miss
				{
					...
					{
						stats . total_dist_reqs += m_table_cost[address_to_id(m_TLB.translate(r_ICACHE_MISS_ADDR_SAVE.read()))];
						stats . total_dist_virt_reqs += m_table_cost[address_to_id(r_ICACHE_MISS_ADDR_SAVE.read())];
						stats . total_reqs += 1;
					}
#endif
		...
		case REQ_DMISS :
			...
			{ 
#ifdef COHERENT_XCACHE_DEBUG
				file_chrono << std::dec << ncycles << " " << m_i_ident << " : " << " DATA-R @0x" << std::hex <<  (r_REQ_DCACHE_ADDR_VIRT & m_dcache_yzmask )
					<< " page4k P_" << std::dec << ((r_REQ_DCACHE_ADDR_VIRT & 0x00FFFFFF )>>12) << " pc_0x" << std::hex << icache_address;
#endif
				...
				if (p_i_vci.cmdack.read())
				{
					...
					if (r_REQ_CPT == r_WRITE_BURST_SIZE - 1)
					{
						...
						file_chrono << std::dec << ncycles << " " << m_i_ident << " : " << " DATA-W BURST SIZE " << std::dec  << r_WRITE_BURST_SIZE.read()
							<< std::hex <<  " @0x" << std::hex <<  (r_REQ_DCACHE_ADDR_VIRT & m_dcache_yzmask )
							<< " page4k P_" << std::dec << ((r_REQ_DCACHE_ADDR_VIRT & 0x00FFFFFF )>>12) << " pc_0x" << std::hex << icache_address;
#endif
					}
					else
...
// Manage Write-buffer through two commands (get and put)
	...
	switch ((rsp_fsm_state_e)r_VCI_RSP_FSM.read()) {
...
#ifdef USE_STATS
			stats.icache_tlb_delay_chunck++;
			stats.icache_tlb_delay++;
#endif
			if (p_i_vci.rspval.read()) { 
#ifdef USE_STATS
				stats.icache_tlb_delay_chunck = 0;
#endif
		...
		case RSP_TLB_DMISS :
			{
#ifdef USE_STATS
				stats.dcache_tlb_delay_chunck++;
				stats.dcache_tlb_delay++;
#endif
				if (p_i_vci.rspval.read())
				{ 
...
#ifdef USE_STATS
					stats.dcache_tlb_delay_chunck = 0;
#endif
					if (p_i_vci.rerror.read() == vci_param::ERR_NORMAL) // Not nack on the tlb miss
					{
